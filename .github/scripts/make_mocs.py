...
class MocMaker:
    ...
    def make_moc_for_directory_with_delimiters(self, root, dirs, files):
        result = ''
        result += MocDelimiter().moc_initial_delimiter()
        result += self.make_moc_for_directory(root, dirs, files)
        result += MocDelimiter().moc_final_delimiter()
        return result
...
def update_existing_moc(initial_content, new_moc_content_with_delimiters):
    ...
    for line in initial_content:
        ...
        if line == MocDelimiter().moc_initial_delimiter():
            inside_old_index = True
            result += new_moc_content_with_delimiters
            index_written = True
            continue

        if line == MocDelimiter().moc_final_delimiter():
            inside_old_index = False
            continue
